    ...
    const handleSubmit = async () => {
        try {
            ...
            if (type === 'changeStatus') {
                res = await iconService.changeStatus(selectedIcon.id);
            } else if (type === 'deleteSoft') {
                res = await iconService.delete(selectedIcon.id);
            } else if (type === 'deleteHard') {
                res = await iconService.delete(selectedIcon.id, false);
            } else {
                notify({ success: false, message: 'Error' });
                return;
            }
        ...
        <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
            ...
            <DialogContent>
                <Typography variant="body1" color="text.secondary">
                    {type === 'deleteSoft'
                        ? `Bạn có chắc chắn muốn xoá mềm sản phẩm: ${selectedIcon?.name}?`
                        : type === 'deleteHard'
                            ? `Bạn có chắc chắn muốn xoá cứng sản phẩm: ${selectedIcon?.name}?`
                            : `Bạn có chắc chắn muốn thay đổi trạng thái sản phẩm: ${selectedIcon?.name}?`}
                </Typography>
            </DialogContent>
