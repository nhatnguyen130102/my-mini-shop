    ...
    const handleSubmit = async () => {
        try {
            ...
            if (type === 'changeStatus') {
                res = await colorService.changeStatus(selectedColor.id);
            } else if (type === 'deleteSoft') {
                res = await colorService.delete(selectedColor.id);
            } else if (type === 'deleteHard') {
                res = await colorService.delete(selectedColor.id, false);
            } else {
                notify({ success: false, message: 'Error' });
                return;
            }
        ...
        <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
            ...
            <DialogContent>
                <Typography variant="body1" color="text.secondary">
                    {type === 'deleteSoft'
                        ? `Bạn có chắc chắn muốn xoá mềm sản phẩm: ${selectedColor?.name}?`
                        : type === 'deleteHard'
                            ? `Bạn có chắc chắn muốn xoá cứng sản phẩm: ${selectedColor?.name}?`
                            : `Bạn có chắc chắn muốn thay đổi trạng thái sản phẩm: ${selectedColor?.name}?`}
                </Typography>
            </DialogContent>
