    ...
    const handleSubmit = async () => {
        try {
            ...
            if (type === 'changeStatus') {
                res = await userService.changeStatus(selectedUser.id);
            } else if (type === 'deleteSoft') {
                res = await userService.delete(selectedUser.id);
            } else if (type === 'deleteHard') {
                res = await userService.delete(selectedUser.id, false);
            } else {
                notify({ success: false, message: 'Error' });
                return;
            }
        ...
        <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
            ...
            <DialogContent>
                <Typography variant="body1" color="text.secondary">
                    {type === 'deleteSoft'
                        ? `Bạn có chắc chắn muốn xoá mềm sản phẩm: ${selectedUser?.name}?`
                        : type === 'deleteHard'
                            ? `Bạn có chắc chắn muốn xoá cứng sản phẩm: ${selectedUser?.name}?`
                            : `Bạn có chắc chắn muốn thay đổi trạng thái sản phẩm: ${selectedUser?.name}?`}
                </Typography>
            </DialogContent>
