    ...
    const handleSubmit = async () => {
        try {
            ...
            if (type === 'changeStatus') {
                res = await materialService.changeStatus(selectedMaterial.id);
            } else if (type === 'deleteSoft') {
                res = await materialService.delete(selectedMaterial.id);
            } else if (type === 'deleteHard') {
                res = await materialService.delete(selectedMaterial.id, false);
            } else {
                notify({ success: false, message: 'Error' });
                return;
            }
        ...
        <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
            ...
            <DialogContent>
                <Typography variant="body1" color="text.secondary">
                    {type === 'deleteSoft'
                        ? `Bạn có chắc chắn muốn xoá mềm sản phẩm: ${selectedMaterial?.name}?`
                        : type === 'deleteHard'
                            ? `Bạn có chắc chắn muốn xoá cứng sản phẩm: ${selectedMaterial?.name}?`
                            : `Bạn có chắc chắn muốn thay đổi trạng thái sản phẩm: ${selectedMaterial?.name}?`}
                </Typography>
            </DialogContent>
