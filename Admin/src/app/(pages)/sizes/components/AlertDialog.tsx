...
const AlertDialog = ({ open, onClose, type }: AlertDialogProps) => {
    const { selectedSize: selectedSize } = useSizeStore();
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (type === 'changeStatus') {
                res = await sizeService.changeStatus(selectedSize.id);
            } else if (type === 'deleteSoft') {
                res = await sizeService.delete(selectedSize.id);
            } else if (type === 'deleteHard') {
                res = await sizeService.delete(selectedSize.id, false);
            } else {
                notify({ success: false, message: 'Error' });
                return;
            }
        ...
        <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
            ...
            <DialogContent>
                <Typography variant="body1" color="text.secondary">
                    {type === 'deleteSoft'
                        ? `Bạn có chắc chắn muốn xoá mềm sản phẩm: ${selectedSize?.name}?`
                        : type === 'deleteHard'
                            ? `Bạn có chắc chắn muốn xoá cứng sản phẩm: ${selectedSize?.name}?`
                            : `Bạn có chắc chắn muốn thay đổi trạng thái sản phẩm: ${selectedSize?.name}?`}
                </Typography>
            </DialogContent>
